...
export class WeeklyTrendService {
  ...
  static async generateWeeklyTrendOverview(request: WeeklyTrendRequest = {}): Promise<WeeklyTrendResponse> {
    try {
      ...
      const url = `${API_BASE_URL}/api/v1/weekly-trend/generate${queryParams.toString() ? '?' + queryParams.toString() : ''}`;
  ...
  static async getTopicKeywords(request: TopicKeywordsRequest = {}): Promise<TopicKeywordsResponse> {
    try {
      ...
      if (request.language) queryParams.append('language', request.language);
      if (request.max_keywords) queryParams.append('max_keywords', request.max_keywords.toString());

      const url = `${API_BASE_URL}/api/v1/topic-keywords${queryParams.toString() ? '?' + queryParams.toString() : ''}`;
